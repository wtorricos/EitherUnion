# Copilot instructions for Results
...
- This is a .NET library that targets `net11.0` preview tooling in `src\WTorricos.Either\WTorricos.Either.csproj` and packs as `WTorricos.Either`.
...
- `src\WTorricos.Either\IEitherExtensions.cs` contains the main fluent API: `Map`, `FlatMap`, `Flatten`, `MapFailure`, `Inspect`, `Filter`, `FromNullable`, `GetValueOrThrow`, and `Void`.
...
- `src\WTorricos.Either\EitherNullFailure.cs` provides the fallback failure used when a null `IEither<T>` is encountered.
