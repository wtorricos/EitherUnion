...
public sealed class RefundPaymentEndpointTest
{
    ...
    [Fact(DisplayName = "HandleV2Async returns status 499 when request is canceled")]
    public async Task HandleV2AsyncReturns499WhenCanceled()
    {
        MethodInfo? handleAsyncMethod = typeof(RefundPaymentV2Endpoint).GetMethod("HandleV2Async", BindingFlags.NonPublic | BindingFlags.Static);
        Assert.NotNull(handleAsyncMethod);
